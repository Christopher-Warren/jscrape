...
export async function scrapeJobsNoAuth(page) {
  ...
  try {
    ...
    await page.goto(
      "https://www.linkedin.com/jobs/search?keywords=chicken%20butcher&location=United%20States&geoId=103644278&trk=public_jobs_jobs-search-bar_search-submit&position=1&pageNum=0"
    );
...
async function updateJobs(page, i, { jobs, excludedJobs }) {
  ...
  const jobListing = await page.waitForSelector(
    `ul:nth-child(1) li:nth-child(${i}) a`,
    { timeout: 5000 }
  );
